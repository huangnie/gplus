    ...
    xhr.addEventListener('readystatechange', function(event) {
        if (xhr.readyState == 4) {
            if (xhr.status == 200) {
                ...
                console.log('clear');
                $.ambiance({message: "Login fail. Please try again.", type: "error", timeout: 2});
...
OAuth2.adapter('google', {
    authorizationCodeURL: function(config) {
        console.log(config);
        return ('https://accounts.google.com/o/oauth2/auth?' + 'client_id={{CLIENT_ID}}&' + 'redirect_uri={{REDIRECT_URI}}&' + 'scope={{API_SCOPE}}&' + 'access_type=offline&' + 'response_type=code').replace('{{CLIENT_ID}}', config.clientId).replace('{{REDIRECT_URI}}', this.redirectURL(config)).replace('{{API_SCOPE}}', config.apiScope);
    },
